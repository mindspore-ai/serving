...
.. py:function:: mindspore_serving.server.distributed.declare_servable(rank_size, stage_size, with_batch_dim=True, without_batch_dim_inputs=None, enable_pipeline_infer=False)
    ...
    参数：
        ...
        - **enable_pipeline_infer** (bool, optional) - 是否开启流水线并行推理，流水线并行可有效提升推理性能，详情可参考 `流水线并行 <https://www.mindspore.cn/tutorials/experts/zh-CN/master/parallel/pipeline_parallel.html>`_ 。默认值：False。
