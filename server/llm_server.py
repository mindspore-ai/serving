...
class LLMServer:
     ...
     def start_background_loop_back(self) -> None:
        # todo
        ...
        running_tasks.add(self.background_loop)
        self.background_loop.add_done_callback(lambda t: running_tasks.remove(t))

     async def register_request(
        self,
        request_id: str,
        prompt: Optional[str]) -> AsyncResultsOfOneRequest:
     ...
     async def step(self):
        ...
        logging.info("get {} new_requests".format(len(new_requests)))
        if len(finished_requests) != 0:
            logging.info("get {} finished_requests".format(len(finished_requests))) 
        ...
        if request_outputs is not None:
            logging.info("get {} response results.".format(len(request_outputs)))
     ...
     async def generate_answer(
            self,
            prompt: Optional[str],
            request_id: str) -> ResponseOutput:
        
        # Preprocess the request.
        try:
            res_stream = await self.register_request(request_id, prompt)
