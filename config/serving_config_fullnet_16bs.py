...
prefill_model_path = [
    "/home/renyujin/ft-predict-opt/mindformers/research/baichuan2/baichuan2_13b_bs1_seq4096_pa224/baichuan2_13b_prefill_graph.mindir"
]
...
def baichuan2_pa_inputs_for_warmup(seq_length, batch_size, full_model, valid_length=None):
    ...
    full_seq_len = Baseconfig.seq_length[0]
    ...
    block_tables = np.full((batch_size, full_seq_len // 128), -1, dtype=np.int32)
    logging.debug("block tables shape: %s, seq_length is: %s", block_tables.shape, full_seq_len)
    for i in range(batch_size):
        block_tables[i][0] = 1
    if full_model:
