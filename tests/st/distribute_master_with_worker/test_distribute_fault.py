...
def test_distribute_fault():
    ...
    assert np.allclose(ret, 0)
    ret = os.system(f"sh {sh_path}/kill_15_master.sh {folders[-1].split('mindspore', 1)[0] + 'mindspore'}")
    assert np.allclose(ret, 0)
    ret = os.system(f"sh {sh_path}/kill_9_master.sh {folders[-1].split('mindspore', 1)[0] + 'mindspore'}")
    assert np.allclose(ret, 0)
if __name__ == '__main__':
    test_distribute_fault()
